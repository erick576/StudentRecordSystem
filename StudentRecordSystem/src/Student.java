...
// Accessor Method to get the grade that the student currently has in the course
	public double getCourseGrade(Course c) {
		double e = -1; // Initialize a variable to hold the default grade in case the student doesn't
						// have a grade yet
...
// Accessor Method to assign or change the grade of the student
	public void setCourseGrade(Student s, double CourseGrade, String CourseName) { // Need the student, grade and name
																					// of course to change the grade
...
// Mutator Method to add a course for a student
	void addCourse(Course c) throws CourseCapacityReached { // In case of a student trying to add over the course limit,
															// an exception will be thrown that wont allow the student
															// to do so
		if (noc == 12) { // The limit is 12
			throw new CourseCapacityReached("Maximum Allowed Courses has already been reached"); // Student will receive
																									// this message
		} else {
			Course cc = new Course(c.getCourseName(), c.getFee(), c.getAoc(), c.getProf()); // If allowed to add course
																							// the course from the
																							// system will be duplicated
																							// so that the students
																							// grades can be unique
...
// Accessor Method that returns the GPA of the student in a 9.0 scale
	double getGPA() {
		...
		int cwg = 0; // Initialize a variable that will store the courses the student is taking with
						// grades
		for (int i = 0; i < noc; i++) { // Run through all the courses that the Student has
			if (courses[i].CourseGrade != -1) { // Only add to the variable when the course has a grade associated to it
												// (-1 means it doesn't have a grade)
